...
int rotationalServoPin = 9;
int rotationalServoSpeed = 1475;
...
void loop(void) {
  rotationalServo.write(1488);
  rgb_sensor.getData();
 
  if (rgb_sensor.r < 14000 || rgb_sensor.r > 20000){
    if (redAvg != 0 && greenAvg != 0 && blueAvg != 0 && cAvg != 0){ //If AVG's aren't empty, we know we just scanned a bean
    ...
    tempRed = tempRed + rgb_sensor.r;  
    tempGreen = tempGreen + rgb_sensor.g;
    tempBlue = tempBlue + rgb_sensor.b;
    tempC = tempC + rgb_sensor.c;
    readCount++;
    redAvg = tempRed/readCount;
    greenAvg = tempGreen/readCount;
    blueAvg = tempBlue/readCount; 
    cAvg = tempC/readCount;
     
    printValues(rgb_sensor);
  }
  delay(60);
}

void printValues(tcs34725 rgb_sensor){
    Serial.print(F("Gain:"));
